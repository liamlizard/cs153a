...
int main() {
	...
	for (i = 0; i < NUMBER_OF_TRIALS; i++) {
		...
		//volatile float a = (float)rand() / (float)rand();
		//b = (float)rand() / (float)rand();

		//volatile int onOff = rand() % 2;

		//Addr = rand() % BUFFER_SIZE; //Will be used to access a random buffer index

		volatile char* word = "ABCDEFGHIJ";
		for(int j = 0; j < 10; j++){
			word[j] = 32 + rand()% 95;
		}
		...
		REPEAT_5(xil_printf("%s\n\r", word));

		//REPEAT_100(c = a + b);


		//REPEAT_100(XGpio_DiscreteWrite(&Gpio, LED_CHANNEL, onOff)); //Turns on one LED
	...
	for (i = 0; i < NUMBER_OF_TRIALS; i++) {
		xil_printf("%d\r", numClockCycles[i]);
	}
